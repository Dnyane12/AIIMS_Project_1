...
public class AccountReceivablePage {
	WebDriver driver;
	private static final Logger logger = LogManager.getLogger(AccountReceivablePage.class);	
		...
		@FindBy(xpath="(//igx-nav-drawer[contains(@id,'project-menu')]//span[contains(normalize-space(.),'Account Receivable')])[1]")
		private WebElement accountRecPageLink;
		...
		public void clickAccountRecLink() {
			WaitHelper.waitForInvisibilityOfElementLocated(driver, dotSpinner, 20);
			WaitHelper.waitForClickable(driver,accountRecPageLink, 20);
			accountRecPageLink.click();
		}
