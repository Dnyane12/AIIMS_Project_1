...
public class DropdownUtils{
 ...
 // Select dropdown option by visible text without typing into input
	public static void selectFromComboWithoutSearch(WebDriver driver, WebDriverWait wait, String labelText, String optionText) {
    	try {
            ...
            //short.until(ExpectedConditions.elementToBeClickableElementLocated(By.xpath("//div[contains(@class,'igx-overlay__content')]")));

            //optionText = optionText.trim();  

            // Construct dropdown option XPath
            String optionXpath = String.format("//div[contains(@class,'igx-overlay__content')]//igx-combo-item[@role='option']//span[contains(text(),'%s')]", optionText.trim());
            ...
            throw new RuntimeException("Timeout: Could not locate dropdown or option '" + optionText + "' for label '" + labelText + "'.", e);
        } catch (Exception e) {
            e.printStackTrace();
            throw new RuntimeException("Dropdown selection failed for label '" + labelText + "' and option '" + optionText + "'.", e);
        }
    }
}
